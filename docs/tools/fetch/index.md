...
When Docker Desktop is running, remote HTTP(S) agent configuration sources and built-in HTTP toolsets route eligible public destinations through its PAC proxy before the normal environment proxy policy. A PAC `DIRECT` result connects directly. Set `DOCKER_AGENT_DISABLE_DESKTOP_PROXY=1` (or `true`, `yes`, or `on`) to opt out per request and restore standard `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` routing; `NO_PROXY` does not bypass Desktop PAC selection. Loopback always stays direct. Guarded clients use PAC only when local DNS yields one or more public addresses. Only a DNS name-not-found error may be delegated to PAC; other lookup errors, empty results, and private or mixed answers remain on the SSRF-protected direct path.

For `allow_private_ips: true`, Docker Desktop PAC still comes first for eligible non-loopback destinations; loopback remains direct. After opting out, normal environment-proxy routing applies.
...
Set `DOCKER_AGENT_DISABLE_DESKTOP_PROXY=1` (or `true`, `yes`, or `on`) to bypass only Docker Desktop's PAC adapter temporarily when diagnosing egress problems. The value is read for each request. This does not disable SSRF protection; use the tool's documented `allow_private_ips: true` option only for trusted internal services.
