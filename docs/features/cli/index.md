...
| `--session-workingdir-root <path>`      | Confine the `working_dir` of sessions created through the `--listen` control plane to this directory and its descendants (default: no restriction — any host directory is accepted). Recommended when the control plane is reachable by other users. Hidden from `--help`, like `--listen`. |
...
| `--session-workingdir-root <path>` | (none)     | Confine the `working_dir` of sessions created via `POST /api/sessions` to this directory and its descendants (symlinks are resolved before the check). Unrestricted by default — any host directory is accepted, as local single-user daemons rely on. Recommended for multi-user or network-exposed deployments. |
