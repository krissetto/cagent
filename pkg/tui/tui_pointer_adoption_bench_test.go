...
type pointerBurstMetrics struct {
	inputMessages, updates, motion, wheel, views, compositions uint64
...
func runPointerBurst(t *testing.T, wheel, motion int) pointerBurstMetrics {
	...
	mouseCoalescer.SetSender(program.Send)
	...
	metrics := pointerBurstMetrics{
		inputMessages: uint64(wheel + motion), updates: model.updates.Load(), motion: model.motion.Load(), wheel: model.wheel.Load(),
