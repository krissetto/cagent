...
func TestHandleAttachSessionHydratesEmptyChildFromStoreWithoutAttach(t *testing.T) {
	ctx := t.Context()
	store := session.NewInMemorySessionStore()
	root := session.New(session.WithID("root"))
...
func TestHandleAttachSessionRequiresStoredChildSession(t *testing.T) {
	ctx := t.Context()
	root := session.New(session.WithID("root"))
	liveChild := session.NewRuntimeManagedSubSession(root, session.WithID("child"))
...
func TestHandleAttachSessionRequiresStoredSessionWhenNoHistory(t *testing.T) {
	ctx := t.Context()
	root := session.New(session.WithID("root"))
...
func TestHandleAttachSessionOpensNonLiveStoredChildInteractiveWithTitle(t *testing.T) {
	ctx := t.Context()
	store := session.NewInMemorySessionStore()
	root := session.New(session.WithID("root"))
	require.NoError(t, store.AddSession(ctx, root))
...
func TestHandleAttachSessionUsesStoredTitleAndTitleEventsUpdateTab(t *testing.T) {
	ctx := t.Context()
	store := session.NewInMemorySessionStore()
	root := session.New(session.WithID("root"))
	require.NoError(t, store.AddSession(ctx, root))
	storedChild := session.NewRuntimeManagedSubSession(root, session.WithID("child"), session.WithTitle("Stored Child Title"), session.WithAgentName("greppy"))
	require.NoError(t, store.AddSubSession(ctx, "root", storedChild))
