...
func renderSubagentAction(verb, agentName, id string) string {
	...
	switch {
	case agentName != "" && id != "":
		parts = append(parts, styles.AgentAccentStyleFor(agentName).Render(agentName)+styles.MutedStyle.Render(" ("+id+")"))
	case agentName != "":
		parts = append(parts, styles.AgentAccentStyleFor(agentName).Render(agentName))
	case id != "":
