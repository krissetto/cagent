...
func TestExpandAllAttachments_WithPastes(t *testing.T) {
	t.Parallel()
	...
	result := e.expandAllAttachments(input)

	// Placeholder stays in message, content in attachments section
	assert.True(t, strings.HasPrefix(result, "Hello "+att.placeholder+" world"), "placeholder should stay in message")
	assert.Contains(t, result, "<attachments>")
	assert.Contains(t, result, "<"+att.placeholder+">")
	assert.Contains(t, result, content)
	assert.Contains(t, result, "</"+att.placeholder+">")
	assert.NoFileExists(t, att.path, "paste file should be removed after expansion")
	assert.Empty(t, e.attachments, "attachments should be cleared")
}

func TestExpandAllAttachments_RemovesUnusedFiles(t *testing.T) {
	t.Parallel()
	...
	// Expand with content that doesn't include the placeholder
	result := e.expandAllAttachments("no placeholder here")
...
func TestFormatBytes(t *testing.T) {
	...
	tests := []struct {
		...
		{"kilobytes_fractional", 1536, "1.5 KB"},
		{"megabytes", 1024 * 1024, "1.0 MB"},
		{"megabytes_fractional", 1536 * 1024, "1.5 MB"},
	}
...
func createPasteAttachmentInDir(dir, content string) (attachment, error) {
	...
	return attachment{
		path:        path,
		placeholder: "@" + path,
		label:       filepath.Base(path) + " (" + formatBytes(len(content)) + ")",
		isTemp:      true,
	}, nil
}
