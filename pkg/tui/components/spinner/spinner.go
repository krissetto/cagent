...
func New(runtime *animation.Runtime, mode Mode, dotsStyle lipgloss.Style) Spinner {
	return NewWithAnimation(runtime, mode, dotsStyle, animation.Chat)
}

// NewWithFrames creates a spinner that animates using the provided frame set.
// If frames is empty, animation.Chat is used.
func NewWithFrames(runtime *animation.Runtime, mode Mode, dotsStyle lipgloss.Style, frames animation.Frames) Spinner {
	if len(frames) == 0 {
		return NewWithAnimation(runtime, mode, dotsStyle, animation.Chat)
	}
	return NewWithAnimation(runtime, mode, dotsStyle, animation.NewSpinner(frames, spinnerFrameDuration))
}
...
func NewWithAnimation(runtime *animation.Runtime, mode Mode, dotsStyle lipgloss.Style, anim animation.Spinner) Spinner {
	if runtime == nil {
...
func (s *spinner) Reset() Spinner {
	return NewWithAnimation(s.runtime, s.mode, s.dotsStyle, s.spinnerAnim)
}
...
func (s *spinner) SetMessage(msg string) {
	...
	s.lastLightStep = animation.TimedStep(s.runtime.Now(), lightStepDuration)
...
func (s *spinner) Update(message tea.Msg) (layout.Model, tea.Cmd) {
	if tick, ok := message.(animation.TickMsg); ok {
		...
		if s.mode == ModeBoth {
			targetStep := animation.TimedStep(s.runtime.Now(), lightStepDuration)
...
func (s *spinner) RawFrame() string {
	return s.spinnerFrames[s.spinnerAnim.FrameIndexAt(s.runtime.Now())]
}

func (s *spinner) View() string {
	frame := s.spinnerAnim.FrameIndexAt(s.runtime.Now())
