...
func TestSidebar_StreamAndTurnLifecycleSplit(t *testing.T) {
	...
	assert.True(t, m.needsSpinner(),
		"a live stream must keep the shared spinner ticking even before the first TurnStartedEvent")
	...
	_, _ = m.Update(runtime.StreamStopped("session-1", "root", "normal"))
	assert.Empty(t, m.currentSessionID, "StreamStoppedEvent should clear live-stream spinner state")
	assert.False(t, m.needsSpinner(), "stopped stream with no other active work should not keep spinner alive")
}
