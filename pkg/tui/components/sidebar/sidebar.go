...
func New(sessionState *service.SessionState) Model {
	...
	m := &model{
		...
		childQueues:        make(map[string][]string),
		childQueueAgents:   make(map[string]string),
		subagentClickZones: make(map[int]string),
		parentClickZone:    -1,
		subagentSpinners:   make(map[string]spinner.Spinner),
...
func (m *model) SetLiveSessionTree(tree *runtime.LiveSessionTree) {
	m.liveSessionTree = tree
	if tree != nil && tree.Root != nil {
		if m.parentSessionID != "" {
			if agent := liveSessionAgentName(tree.Root, m.parentSessionID); agent != "" {
				m.parentAgentName = agent
			}
		}
		m.seedSubagentSpinners(tree.Root)
		for id := range m.childQueues {
			if agent := liveSessionAgentName(tree.Root, id); agent != "" {
				m.childQueueAgents[id] = agent
			}
		}
	}
	m.invalidateCache()
...
func (m *model) SetPersistedSessionTree(root *session.Session, store session.Store) {
	m.setParentSessionInfo(context.Background(), root, store)
	if tree := persistedLiveSessionTree(context.Background(), root, store); tree != nil {
		m.SetLiveSessionTree(tree)
	}
}

func (m *model) setParentSessionInfo(ctx context.Context, sess *session.Session, store session.Store) {
	...
	m.parentSessionID = sess.ParentID
	m.parentAgentName = ""
	if store != nil {
		if parent, err := store.GetSession(ctx, sess.ParentID); err == nil && parent != nil {
			m.parentAgentName = persistedSessionAgentName(parent)
		}
	}
	if m.parentAgentName == "" {
		m.parentAgentName = "parent"
		if sess.ParentID == sess.EffectiveRootID() {
			m.parentAgentName = "root"
		}
	}
	m.invalidateCache()
}

func parentSessionAgentLabel(agentName string) string {
	if agentName == "" {
		return "parent"
	}
	return agentName
...
func (m *model) setChildQueue(event *runtime.SessionQueueEvent) {
	...
	if event.Count <= 0 {
		delete(m.childQueues, event.SessionID)
		delete(m.childQueueAgents, event.SessionID)
		m.invalidateCache()
		return
	}
	previews := append([]string(nil), event.Previews...)
	if len(previews) == 0 {
		previews = []string{fmt.Sprintf("%d queued", event.Count)}
	}
	m.childQueues[event.SessionID] = previews
	if agent := liveSessionAgentName(m.liveSessionTree.Root, event.SessionID); agent != "" {
		m.childQueueAgents[event.SessionID] = agent
	}
	m.invalidateCache()
}
...
const (
	...
	ClickSubagent   // Click on a runtime-managed subagent row
	ClickParent     // Click on the attached session's parent row
)
...
func (m *model) HandleClickType(x, y int) (ClickResult, string) {
	...
	// Parent session row sits directly under the working directory in attached sub-sessions.
	if m.parentSessionID != "" && contentY == m.parentLineY() {
		return ClickParent, m.parentSessionID
	}

	// Check if click is on a subagent row
	if id, ok := m.subagentClickZones[contentY]; ok {
		return ClickSubagent, id
	}
...
func (m *model) Update(msg tea.Msg) (layout.Model, tea.Cmd) {
	...
	case tea.MouseClickMsg, tea.MouseMotionMsg, tea.MouseReleaseMsg, messages.WheelCoalescedMsg:
		if m.mode == ModeVertical {
			if motion, ok := msg.(tea.MouseMotionMsg); ok {
				hoverChanged := m.updateParentHoverAt(motion.X, motion.Y)
				if m.updateSubagentHoverAt(motion.X, motion.Y) || hoverChanged {
					m.invalidateCache()
				}
			}
	...
	case *runtime.SubAgentStartedEvent:
		...
		cmd := m.subagentSpinnerCmd(msg.SubAgent.ID, msg.SubAgent.AgentName, true)
		return m, cmd
	case *runtime.SubAgentSentEvent:
		m.invalidateCache()
		cmd := m.subagentSpinnerCmd(msg.SubAgentID, "", true)
		return m, cmd
	case *runtime.SubAgentUpdateEvent:
		cmd := m.recordSubAgentUpdate(msg)
		return m, cmd
	case *runtime.LiveSessionTreeChangedEvent:
		m.SetLiveSessionTree(msg.Tree)
		return m, nil
	...
	case messages.ThemeChangedMsg:
		...
		// Recreate all subagent spinners with their generated per-agent colors.
	...
	default:
		...
		// Update subagent spinners.
		for id, sp := range m.subagentSpinners {
			model, cmd := sp.Update(msg)
			m.subagentSpinners[id] = model.(spinner.Spinner)
			cmds = append(cmds, cmd)
			needsInvalidate = true
		}
...
func (m *model) renderSections(contentWidth int) []string {
	...
	appendSection(m.childQueueSection(contentWidth))

	subagentSectionStart := len(lines)
	appendSection(m.subagentsSection(contentWidth))
...
func (m *model) sessionInfo(contentWidth int) string {
	...
	if m.parentSessionID != "" {
		lines = append(lines, m.parentSessionLine())
	}

	return m.renderTab("Session", strings.Join(lines, "\n"), contentWidth)
}

func (m *model) parentSessionLine() string {
	agent := parentSessionAgentLabel(m.parentAgentName)
	labelStyle := styles.MutedStyle
	agentStyle := styles.AgentAccentStyleFor(agent)
	if m.hoveredParent {
		labelStyle = styles.Hovered(labelStyle)
		agentStyle = styles.Hovered(agentStyle)
	}
	return labelStyle.Render("parent: ") + agentStyle.Render(agent)
}

func (m *model) parentLineY() int {
	y := verticalStarY + m.titleLineCount() + 1
	if m.workingDirectory != "" {
		y++
	}
	return y
...
func liveSessionAgentName(node *runtime.LiveSessionNode, id string) string {
	if node == nil {
		return ""
	}
	if node.ID == id {
	...
	for _, child := range node.Children {
		if agent := liveSessionAgentName(child, id); agent != "" {
			return agent
		}
	}
	return ""
...
func (m *model) childQueueSection(contentWidth int) string {
	if len(m.childQueues) == 0 {
		return ""
	}
	ids := make([]string, 0, len(m.childQueues))
	for id := range m.childQueues {
		ids = append(ids, id)
	}
	slices.Sort(ids)
	...
	var lines []string
	for _, id := range ids {
		label := sidebarShortID(id)
		if agent := m.childQueueAgents[id]; agent != "" {
			label = styles.SubagentAccentStyleFor(agent).Render(agent) + styles.MutedStyle.Render(" ("+sidebarShortID(id)+")")
		}
		lines = append(lines, styles.MutedStyle.Render("┌ ")+label)
		previews := m.childQueues[id]
		for i, preview := range previews {
			prefix := "├ "
			if i == len(previews)-1 {
				prefix = "└ "
			}
			lines = append(lines, styles.MutedStyle.Render(prefix)+toolcommon.TruncateText(preview, maxMsgWidth))
		}
	}
	return m.renderTab("Child Queue", strings.Join(lines, "\n"), contentWidth)
}

func (m *model) subagentsSection(contentWidth int) string {
...
func (m *model) seedSubagentSpinners(root *runtime.LiveSessionNode) {
	...
	walk = func(node *runtime.LiveSessionNode) {
		...
		if node.ID != root.ID {
			...
			} else if sp, ok := m.subagentSpinners[node.ID]; ok {
				sp.Stop()
				delete(m.subagentSpinners, node.ID)
			}
		}
	...
	for id, sp := range m.subagentSpinners {
		if !seen[id] {
			sp.Stop()
			delete(m.subagentSpinners, id)
		}
	}
}

func (m *model) ensureSubagentSpinner(id, agent string) spinner.Spinner {
...
func (m *model) subagentSpinnerCmd(id, agent string, working bool) tea.Cmd {
	if id == "" {
		return nil
	}
	if working {
		sp := m.ensureSubagentSpinner(id, agent)
		m.subagentSpinners[id] = sp
		m.invalidateCache()
		return sp.Init()
	}
...
func (m *model) subagentSpinnerView(id, agent string) string {
	sp := m.ensureSubagentSpinner(id, agent)
	m.subagentSpinners[id] = sp
	return sp.RawFrame()
}

func (m *model) recordSubAgentStart(event *runtime.SubAgentStartedEvent) {
	if event == nil {
		return
	}
	m.invalidateCache()
...
func (m *model) recordSubAgentUpdate(event *runtime.SubAgentUpdateEvent) tea.Cmd {
	if event == nil {
		return nil
	}
	working := event.Envelope.Status == "running" || event.Envelope.Status == "starting"
	m.invalidateCache()
	return m.subagentSpinnerCmd(event.Envelope.SubAgentID, event.Envelope.AgentName, working)
}

func subagentStatusWorking(node *runtime.LiveSessionNode) bool {
	if node == nil {
		return false
	}
	s := strings.ToLower(node.Status)
	return node.Live && (s == "" || s == "running" || s == "starting")
}

func subagentStatusLabel(node *runtime.LiveSessionNode) string {
	if node == nil {
		return ""
	}
...
func (m *model) buildSubagentClickZones(start int, lines []string) {
	...
	m.parentClickZone = -1
	if m.parentSessionID != "" {
		m.parentClickZone = m.parentLineY()
	}
	if m.liveSessionTree == nil || m.liveSessionTree.Root == nil || len(m.liveSessionTree.Root.Children) == 0 {
		return
	}
...
func (m *model) updateParentHoverAt(x, y int) bool {
	if m.mode != ModeVertical || m.parentSessionID == "" || m.parentClickZone < 0 {
		return m.setHoveredParent(false)
	}
	...
	return m.setHoveredParent(contentY == m.parentClickZone)
}

func (m *model) setHoveredParent(hovered bool) bool {
	if m.hoveredParent == hovered {
		return false
	}
	m.hoveredParent = hovered
	return true
}
