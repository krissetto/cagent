...
type model struct {
	...
	parentIdle         bool // true while parent is idle waiting on subagents
	...
	// Parent line click zones: content line indices that navigate to the parent session.
	parentClickZones map[int]struct{}
...
func (m *model) needsSpinner() bool {
	return m.currentSessionID != "" || m.workingAgent != "" || m.toolsLoading || m.mcpInit || m.titleRegenerating || m.liveSubagentCount() > 0
}
...
func (m *model) HandleClickType(x, y int) (ClickResult, string) {
	...
	if m.mode == ModeCollapsed {
		...
		if parentID := m.collapsedParentSessionClickAt(y); parentID != "" {
			return ClickParent, parentID
		}
	...
	workingDirY := verticalStarY + titleLines + 1
	if m.workingDirectory != "" && contentY == workingDirY {
		return ClickWorkingDir, ""
	}

	if parentID := m.parentSessionClickAt(contentY); parentID != "" {
		return ClickParent, parentID
	}
...
func (m *model) updateParentLineHoverAt(x, y int) bool {
	...
	isParentLine := m.parentSessionClickAt(contentY) != ""
	if m.mode == ModeCollapsed {
		isParentLine = m.collapsedParentSessionClickAt(contentY) != ""
	}
...
func (m *model) parentSessionClickAt(contentY int) string {
	if m.sessionState == nil || !m.sessionState.IsSubSession() || m.sessionState.ParentSessionID() == "" {
		return ""
	}
	if _, ok := m.parentClickZones[contentY]; ok {
		return m.sessionState.ParentSessionID()
	}
	return ""
}

func (m *model) buildParentClickZones(sessionSectionStart int, lines []string) {
	m.parentClickZones = make(map[int]struct{})
	if m.sessionState == nil || !m.sessionState.IsSubSession() || m.sessionState.ParentSessionID() == "" {
		return
	}
	for i := sessionSectionStart; i < len(lines); i++ {
		if strings.Contains(ansi.Strip(lines[i]), "parent:") {
			m.parentClickZones[i] = struct{}{}
		}
	}
}

func (m *model) collapsedParentSessionClickAt(y int) string {
	if m.sessionState == nil || !m.sessionState.IsSubSession() || m.sessionState.ParentSessionID() == "" {
		return ""
	}
	lines := strings.Split(RenderCollapsedView(m.computeCollapsedViewModel(m.contentWidth(false))), "\n")
	if y < 0 || y >= len(lines) {
		return ""
	}
	if strings.Contains(ansi.Strip(lines[y]), "parent:") {
		return m.sessionState.ParentSessionID()
	}
	return ""
}
...
func (m *model) Update(msg tea.Msg) (layout.Model, tea.Cmd) {
	...
	case tea.MouseClickMsg, tea.MouseMotionMsg, tea.MouseReleaseMsg, messages.WheelCoalescedMsg:
		if m.mode == ModeVertical {
			...
			// the cursor. Click/release are routed through handleSidebarClickType
			// by the chat page, so we skip hover on those.
	...
	case messages.StreamCancelledMsg:
		// ESC/cancel only clears parent-stream-local UI state. Runtime-managed
		// subagent lifecycle is owned by runtime SubAgentUpdateEvent delivery
		// and live-tree snapshots; the sidebar must not invent terminal state
		// for children that may continue running after the parent turn stops.
	...
	default:
		...
		// Update main spinner when MCP is initializing, tools are loading, a stream is live, agent is working, title is regenerating, or subagents are live
		if m.mcpInit || m.toolsLoading || (m.currentSessionID != "" && !m.parentIdle) || (m.workingAgent != "" && !m.parentIdle) || m.titleRegenerating || m.liveSubagentCount() > 0 {
...
func (m *model) renderSections(contentWidth int) []string {
	...
	sessionSectionStart := len(lines)
	sessionSection := m.sessionInfo(contentWidth)
	appendSection(sessionSection)
	m.buildParentClickZones(sessionSectionStart, lines)
