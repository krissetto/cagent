...
func (m *model) renderLiveSessionNode(lines *[]string, node *runtime.LiveSessionNode, last bool, prefix string, contentWidth int) {
	...
	status := "○"
	if node.Live {
		status = m.spinner.RawFrame()
	}
	label := node.AgentName
	if label == "" {
		label = sidebarShortID(node.ID)
	}
	line := styles.MutedStyle.Render(prefix+branch) + styles.SubagentAccentStyleFor(node.AgentName).Render(status+" "+label)
	if node.Preview != "" {
		line += styles.MutedStyle.Render(" — ") + toolcommon.TruncateText(node.Preview, contentWidth-lipgloss.Width(prefix+branch)-len(label)-4)
	}
