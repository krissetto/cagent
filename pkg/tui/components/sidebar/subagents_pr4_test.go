...
func TestLiveSessionTreeWaitingStatusStopsSubagentSpinner(t *testing.T) {
	m := New(&service.SessionState{}).(*model)
	m.SetLiveSessionTree(&runtime.LiveSessionTree{Root: &runtime.LiveSessionNode{
		ID:       "root",
		Children: []*runtime.LiveSessionNode{{ID: "child12345", AgentName: "greppy", Status: "running", Live: true, CreatedAt: time.Now()}},
	}})
	if _, ok := m.subagentSpinners["child12345"]; !ok {
		t.Fatalf("expected running live tree child to seed spinner")
	}
	...
	if _, ok := m.subagentSpinners["child12345"]; ok {
		t.Fatalf("expected waiting live tree child to clear spinner")
	}
}
