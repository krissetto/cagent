...
func (m *model) SeedSubagentsFromLiveTree(nodes []runtime.LiveSessionNode) {
	...
	for _, n := range nodes {
		...
		// A live-tree snapshot reporting a non-terminal state is authoritative
		// even if the sidebar currently thinks the row is terminal. This heals
		// attach/reseed races where a persisted child-session row was first
		// classified as "closed" before the runtime manager's live snapshot
		// arrived. Terminal snapshots still keep the old guard: once the UI has
		// observed a real terminal event, stale snapshots must not resurrect it.
		snapshotIsLive := !snapshotStatus.IsTerminal()
		if snapshotIsLive || (!state.Status.IsTerminal() && !existingIsNewer) {
