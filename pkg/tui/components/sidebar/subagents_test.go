...
func TestSubagents_SeedNestedSubagentReload(t *testing.T) {
	...
	nodes := []runtime.LiveSessionNode{
		{SessionID: "root-1", Kind: runtime.LiveSessionRoot, AgentName: "root-agent", Status: "running"},
		{SessionID: "subagent-a", ParentSessionID: "root-1", Kind: runtime.LiveSessionSubAgent, AgentName: "planner", Status: "closed", CreatedAt: now.Add(-2 * time.Minute)},
		{SessionID: "subagent-b", ParentSessionID: "subagent-a", Kind: runtime.LiveSessionSubAgent, AgentName: "reviewer", Status: "closed", CreatedAt: now.Add(-1 * time.Minute)},
	}
...
func TestSubagents_SeedFromLiveTreePopulatesNestedDescendants(t *testing.T) {
	...
	nodes := []runtime.LiveSessionNode{
		...
		{SessionID: "worker-1", Kind: runtime.LiveSessionRoot, AgentName: "worker", Status: "running"},
		// Direct descendants.
		{SessionID: "planner-1", ParentSessionID: "worker-1", Kind: runtime.LiveSessionSubAgent, AgentName: "planner", Status: "running", CreatedAt: now.Add(-3 * time.Minute), LastUpdateAt: now.Add(-1 * time.Minute)},
		{SessionID: "reviewer-1", ParentSessionID: "worker-1", Kind: runtime.LiveSessionSubAgent, AgentName: "reviewer", Status: "waiting", CreatedAt: now.Add(-2 * time.Minute), LastPreview: "preview text"},
		// Grandchild under planner — this is the nested case that wouldn't
		// show up without seeding.
		{SessionID: "leaf-1", ParentSessionID: "planner-1", Kind: runtime.LiveSessionSubAgent, AgentName: "leaf", Status: "failed", Error: "oops", CreatedAt: now.Add(-time.Minute)},
	}
...
func TestSubagents_StreamCancelledKeepsLiveState(t *testing.T) {
	t.Parallel()
	...
	assert.Len(t, m.subagents, 1,
		"ESC should stop the parent spinner but must not wipe live subagent sidebar state")
	state := m.subagents["x"]
	require.NotNil(t, state)
	assert.Equal(t, subagent.StatusClosed, state.Status,
		"stream cancel should mark in-flight subagents as terminal so the spinner stops")
}
...
func TestSubagents_LiveTreeCanonicalizesRestoredShortIDEntry(t *testing.T) {
	...
	// Now seed the live tree using the full session id.
	m.SeedSubagentsFromLiveTree([]runtime.LiveSessionNode{
		{Kind: runtime.LiveSessionRoot, SessionID: "root"},
		{
	...
	assert.Equal(t, subagent.StatusClosed, state.Status,
		"terminal status from restore must be preserved by the seed guard — once a subagent "+
			"reaches a terminal state in the sidebar, a snapshot cannot flip it back")
...
func TestSubagents_LiveTreeRendersGrandchildIndentedUnderChild(t *testing.T) {
	...
	m.SeedSubagentsFromLiveTree([]runtime.LiveSessionNode{
		{Kind: runtime.LiveSessionRoot, SessionID: rootID},
		{
			Kind:            runtime.LiveSessionSubAgent,
			SessionID:       childID,
