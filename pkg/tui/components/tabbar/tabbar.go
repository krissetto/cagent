...
// Height returns the height of the tab bar (single line).
...
func (t *TabBar) View() string {
	...
	for i, info := range t.tabs {
		allTabs[i] = renderTab(info, t.maxTitleLen)
	...
	if needsScroll && t.scrollOffset > 0 {
		arrow := arrowStyle.Render(scrollLeftText)
		line += arrow
	...
	if needsScroll && lastVisibleIdx < len(allTabs)-1 {
		arrow := arrowStyle.Render(scrollRightText)
		line += arrow
		t.zones = append(t.zones, clickZone{
			startX: cursor, endX: cursor + scrollArrowWidth,
