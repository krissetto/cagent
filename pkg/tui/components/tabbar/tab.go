...
const (
	...
	runningIndicator = "● "
	// attentionIndicator is appended after the title when the tab needs attention.
	attentionIndicator = " !"
...
func renderTab(info messages.TabInfo, maxTitleLen int) Tab {
	title := info.Title
	if title == "" {
	...
	content := bar
	if info.IsRunning {
		// Use the theme's highlight color (same as the spinner) so both the
		// tab dot and the in-view spinner share the same "streaming" visual.
		runFg := ensureContrast(styles.TabAccentFg, bgColor)
		content += lipgloss.NewStyle().Foreground(runFg).Background(bgColor).Render(runningIndicator)
	} else {
		content += pad.Render(" ")
	}
