...
func New(ctx context.Context, spawner SessionSpawner, initialApp *app.App, initialWorkingDir string, cleanup func()) tea.Model {
	...
	tabTitleMaxLen := config.GetUserSettings().GetTabTitleMaxLength()
	...
	// Restore previously saved tabs (if enabled) and persist the current state.
	//
	// Key invariant: the tuistate DB always stores **persisted session-store IDs**
	// (the IDs returned by SessionStore.GetSession). Runtime tab/routing IDs are
	// an ephemeral, in-process concept. The pendingRestores map bridges the two:
	//   pendingRestores[runtimeTabID] = persistedSessionID
	if ts != nil {
		restoreTabs := config.GetUserSettings().RestoreTabs
		...
		if len(savedTabs) == 0 {
			// No tabs to restore — start fresh with the initial session.
			if err := ts.ClearTabs(ctx); err != nil {
				slog.Warn("Failed to clear tabs", "error", err)
			}
			if err := ts.AddTab(ctx, sessID, initialWorkingDir); err != nil {
				slog.Warn("Failed to persist initial tab", "error", err)
			}
			if err := ts.SetActiveTab(ctx, sessID); err != nil {
				slog.Warn("Failed to set active tab", "error", err)
			}
		} else {
			// Tabs to restore. We keep the persisted session IDs in the DB
			// untouched — no ClearTabs / AddTab dance. The DB rows stay keyed
			// by the original session-store IDs so that even tabs that are
			// never switched to survive subsequent restarts.

			sessionStore := initialApp.SessionStore()
			restoredFirst := false // tracks whether the first tab slot was filled

			for _, saved := range savedTabs {
				// Validate the saved session still exists in the store.
				...
				if !restoredFirst {
					restoredFirst = true
					// First valid saved tab: load into the initial tab via
					// replaceActiveSession in Init() or lazily on first switch.
					m.pendingRestores[sessID] = saved.SessionID
					if saved.SidebarCollapsed {
						m.pendingSidebarCollapsed[sessID] = true
					}
					...
					// Peek at the session title so the tab bar shows
					// the correct name before the session is fully loaded.
					if sessionStore != nil && saved.SessionID != "" {
						if oldSess, err := sessionStore.GetSession(ctx, saved.SessionID); err == nil && oldSess.Title != "" {
							sv.SetRunnerTitle(sessID, oldSess.Title)
						}
					}
					continue
				}

				// Remaining tabs: spawn a runtime slot. Session data is loaded
				// lazily via replaceActiveSession when the user switches to the tab.
				...
				newID := sv.AddSession(ctx, a, newSess, saved.WorkingDir, spawnCleanup)

				// Stash persisted session ID for lazy loading on first switch.
				m.pendingRestores[newID] = saved.SessionID
				if saved.SidebarCollapsed {
					m.pendingSidebarCollapsed[newID] = true
				}

				// If this was the focused tab, switch to it on Init().
				if saved.SessionID == savedActiveID {
					m.pendingActiveTab = newID
				}

				// Peek at the session title for the tab bar so tabs aren't all "New session".
				if sessionStore != nil && saved.SessionID != "" {
					if oldSess, err := sessionStore.GetSession(ctx, saved.SessionID); err == nil && oldSess.Title != "" {
						sv.SetRunnerTitle(newID, oldSess.Title)
					}
				}
			}

			// If all saved tabs were stale and nothing was restored, persist
			// the initial session so we don't end up with an empty tab store.
			if !restoredFirst {
				if err := ts.ClearTabs(ctx); err != nil {
					slog.Warn("Failed to clear stale tabs", "error", err)
				}
				if err := ts.AddTab(ctx, sessID, initialWorkingDir); err != nil {
					slog.Warn("Failed to persist initial tab after stale cleanup", "error", err)
				}
				if err := ts.SetActiveTab(ctx, sessID); err != nil {
					slog.Warn("Failed to set active tab after stale cleanup", "error", err)
				}
			}
		}
	}
	...
	if oldSessionID, ok := m.pendingRestores[activeID]; ok {
		...
		if store := m.application.SessionStore(); store != nil {
			if sess, err := store.GetSession(context.Background(), oldSessionID); err == nil {
				...
				// Apply persisted sidebar collapsed state for the initial tab
				// and re-layout so sub-component sizes match the collapsed state.
				if collapsed, ok := m.pendingSidebarCollapsed[activeID]; ok {
					m.chatPage.SetSidebarSettings(chat.SidebarSettings{Collapsed: collapsed})
					delete(m.pendingSidebarCollapsed, activeID)
					cmd = tea.Batch(cmd, m.resizeAll())
				}

				// Persist the active tab using the loaded session's persisted ID.
				if m.tuiStore != nil {
...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case messages.BellMsg:
		// Ring the terminal bell to alert the user that a background tab needs attention.
...
func (m *Model) handleLoadSession(sessionID string) (tea.Model, tea.Cmd) {
	...
	if len(currentSess.Messages) == 0 && currentSess.Title == "" {
		...
		if m.tuiStore != nil {
			...
			if err := m.tuiStore.SetActiveTab(ctx, sess.ID); err != nil {
				slog.Warn("Failed to set active tab after in-place load", "error", err)
			}
		}
		return model, cmd
	}
	...
	// Propagate the loaded session's title to the supervisor so the tab bar shows it.
	if sess.Title != "" {
	...
	if m.tuiStore != nil {
		if err := m.tuiStore.SetActiveTab(ctx, sess.ID); err != nil {
			slog.Warn("Failed to set active tab after load", "error", err)
		}
	}
...
func (m *Model) replaceActiveSession(ctx context.Context, sess *session.Session) (tea.Model, tea.Cmd) {
	activeID := m.supervisor.ActiveID()
	...
	// Rebuild session state, chat page, and editor
	m.sessionState = service.NewSessionState(sess)
	m.chatPage = chat.New(m.application, m.sessionState)
	m.editor = editor.New(m.application, m.history)
	...
	m.editors[activeID] = m.editor

	// Propagate the loaded session's title to the tab bar
	if sess.Title != "" {
...
func (m *Model) handleSwitchTab(sessionID string) (tea.Model, tea.Cmd) {
	...
	if oldSessionID, ok := m.pendingRestores[sessionID]; ok {
		...
		if store := runner.App.SessionStore(); store != nil {
			if sess, err := store.GetSession(context.Background(), oldSessionID); err == nil {
				// Persist active tab using the persisted session-store ID.
				if m.tuiStore != nil {
					if err := m.tuiStore.SetActiveTab(context.Background(), sess.ID); err != nil {
						slog.Warn("Failed to set active tab after lazy restore", "error", err)
					}
				}
				model, cmd := m.replaceActiveSession(context.Background(), sess)
				// Apply persisted sidebar collapsed state for lazily restored tabs
				// and re-layout so sub-component sizes match the collapsed state.
				if collapsed, ok := m.pendingSidebarCollapsed[sessionID]; ok {
					m.chatPage.SetSidebarSettings(chat.SidebarSettings{Collapsed: collapsed})
	...
	if !pageExists {
		...
		// Apply persisted sidebar collapsed state for restored tabs.
		if collapsed, ok := m.pendingSidebarCollapsed[sessionID]; ok {
			chatPage.SetSidebarSettings(chat.SidebarSettings{Collapsed: collapsed})
			delete(m.pendingSidebarCollapsed, sessionID)
		}
	...
	if m.tuiStore != nil {
		...
		if err := m.tuiStore.SetActiveTab(context.Background(), persistedID); err != nil {
			slog.Error("Failed to set active tab in store", "error", err)
			cmds = append(cmds, notification.ErrorCmd(fmt.Sprintf("Failed to set active tab in tui state db: %v", err)))
		}
	}
	...
	if !pageExists || !editorExists {
		...
		// Replay pending attention event for new pages too
	...
	// Replay pending attention event (e.g. tool confirmation dialog) that arrived
	// while this tab was in the background and whose dialog command was discarded.
	if pendingCmd := m.replayPendingEvent(sessionID); pendingCmd != nil {
		cmds = append(cmds, pendingCmd)
	}
...
// max iterations, elicitation) that was received while the tab was in the background.
// If found, it opens the appropriate dialog. The event was already processed by the chat page
// (updating the message list), but the dialog command was discarded for background sessions.
func (m *Model) replayPendingEvent(sessionID string) tea.Cmd {
...
func (m *Model) replayElicitationEvent(ev *runtime.ElicitationRequestEvent) tea.Cmd {
...
func (m *Model) persistedSessionID(tabID string) string {
...
func (m *Model) findTabByPersistedID(persistedID string) string {
