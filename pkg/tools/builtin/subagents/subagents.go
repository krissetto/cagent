...
func (s *ToolSet) Tools(context.Context) ([]tools.Tool, error) {
	return []tools.Tool{
		...
		},
		{
			...
			Category:    "subagents",
			Description: "Send a follow-up message to a live runtime-managed subagent. Use this for new information, a follow-up task, or a course correction; let automatic runtime updates handle ordinary progress instead of sending polling messages.",
		...
		},
		{
			...
			Category:    "subagents",
			Description: "List runtime-managed subagents for the current parent session only to recover forgotten IDs or reconstruct context after context loss. Do not use this to poll for progress.",
		...
		},
		{
			...
			Category:    "subagents",
			Description: "Immediately stop a runtime-managed subagent when continuing would be harmful, obsolete, or explicitly unwanted.",
		...
		},
		{
			...
			Category:    "subagents",
			Description: "Ask a runtime-managed subagent to finish cleanly after its current safe point when its work is complete or no longer needed.",
			Parameters:  tools.MustSchemaFor[RefArgs](),
